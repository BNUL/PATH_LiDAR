...
def example_chm_based():
    """示例5b: 基于CHM冠层高度模型的波形模拟"""
    from lidar_simulator_core import CHMWaveformSimulator
    
    print("\n" + "="*60)
    print("示例 5b: 基于CHM的LiDAR波形模拟")
    print("="*60)
    
    # 配置树冠参数
    canopy = CanopyParameters(
        crown_shape='cylinder',  # CHM不需要指定形状
        leaf_area_density=0.35,   # 建议使用0.8-1.0以获得较强回波
        leaf_angle_distribution=6,
        leaf_reflectance=0.35,   # NIR波段典型值
        ground_reflectance=0.35
    )
    ...
    simulator = CHMWaveformSimulator(
        ...
        layer_height=0.15,
        height_threshold=1.0,      # CHM < 1m 判定为间隙
        branch_height_ratio=1/3,   # 枝下高占平均树高的1/3
        manual_Fs0=None,           # None=自动计算
        manual_tree_top_height=None,  # None=使用max(CHM)
        use_pulse_shape=True
    )
    ...
    # 保存结果
    simulator.save_waveform('chm_waveform_output.txt', height, waveform)
...
if __name__ == "__main__":
    ...
    print("5b. example_chm_based()        - 基于CHM冠层高度模型")
